...
harvest.load((err, docsGroup) => {
  const references = content.references;
    harvest._flattenResults(docsGroup, (err, docs) => {
      ...
      content.buildFullRegistry((err) => {
        Async.each(docs, (doc, done) => {
          harvest.Hook.Store(doc, 'Test', (err, doc) => {
          ...
          content.refCollections(doc, primaryCollection, (err, fields) => {
            Async.eachOf(fields, (values, field, fin) => {
              ...
              const ids = content.registry[primaryCollection].reduce((acc, cur, i) => {
                acc.push(Object.values(cur)[0]);
                return acc;
              }, []);
              if (doc[identifierField] in content.registry[primaryCollection]) {
                content.UpdateOne(doc[identifierField], primaryCollection, value, (err, res) => {
                  done();
                });
              }
              else {
                const title = content.getMapFieldByValue(primaryCollection, 'title');
                const interraId = content.buildInterraIdSafe(content.buildInterraId(doc.title), ids);
